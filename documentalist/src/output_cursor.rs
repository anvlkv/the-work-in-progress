...
pub const DEFAULT_X264_OPTS: &str = "preset=veryslow";
...
impl<'s> OutputCursor<'s> {
    ...
    pub fn accept_video_frame(&mut self, frame: &ffmpeg::frame::Video, src_time_base: Rational) -> Result<(), Error> {
        let mut encoder = self.video_encoder.take().unwrap();
        encoder.send_frame(frame)?;
        let dst_time_base = self.output.streams().nth(self.video_stream_index.unwrap()).unwrap().time_base();
    ...
    pub fn accept_audio_frame(&mut self, frame: &ffmpeg::frame::Audio, src_time_base: Rational) -> Result<(), Error> {
        let mut encoder = self.audio_encoder.take().unwrap();
        encoder.send_frame(frame)?;
        let dst_time_base = self.output.streams().nth(self.audio_stream_index.unwrap()).unwrap().time_base();
        Self::receive_packets(
            &mut encoder,
            &mut self.output,
    ...
    pub fn accept_subtitle_frame(&mut self, frame: &ffmpeg::Frame, src_time_base: Rational) -> Result<(), Error> {
        let mut encoder = self.subtitle_encoder.take().unwrap();
        encoder.send_frame(frame)?;
        let dst_time_base = self.output.streams().nth(self.subtitle_stream_index.unwrap()).unwrap().time_base();
        Self::receive_packets(
            &mut encoder,
            &mut self.output,
    ...
    pub fn open_video_encoder(
        ...
        let mut encoder = codec::context::Context::from_parameters(ost.parameters())?
            .encoder()
            .video()?;
        let frame_rate = Some(frame_rate.unwrap_or(rate));
        ...
        unsafe {
            ...
            encoder_video.set_time_base(rate.invert());
            encoder_video.set_frame_rate(frame_rate);
    ...
    pub fn open_audio_encoder(
        ...
        encoder.set_time_base(Rational(1, rate as i32));
    ...
    pub fn open_file(&mut self, meta_data: Dictionary) -> Result<(), Error> {
        self.output.set_metadata(meta_data.to_owned());
        self.output.write_header()?;
        Ok(())
    }

    pub fn close(&mut self, (video_src_time_base, audio_src_time_base, sub_src_time_base): (Option<Rational>, Option<Rational>, Option<Rational>)) -> Result<(), Error> {
...
mod tests {
    ...
    fn it_should_accept_video_frame() {
        ...
        cursor
            .accept_video_frame(&mut frame, Rational(1, 60))
    ...
    fn it_should_accept_audio_frame() {
        ...
        cursor
            .accept_audio_frame(&mut frame, Rational(48000, 1))
    ...
    fn it_should_open_file() {
        ...
        cursor.open_file(meta).expect("error opening file");
    }
    ...
    fn it_should_close() {
        ...
        cursor.close((Some(Rational(1, 60)), Some(Rational(1, 60)), None)).expect("error closing cursor");
    }
}
